...
test("encrypted-poker.ts, multi-player test, shuffle, decrypt every card with all permutations", () => {
  ...
  for (const permutation of allPermutations) {
    // Make a copy of the encrypted card to work with
    // partiallyDecryptedCardLoop_Perm1_1.map((c2, i) => ({
    //   c1: encryptedDeck2[cardIndex][i].c1,
    //   c2,
    // })),
    let currentCard = [...players[0].encryptedDeck[CARD_INDEX]];

    // Apply decryption in the order specified by the permutation
    for (const playerIndex of permutation) {
      currentCard = decryptCard({
        encryptedCard: currentCard,
  ...
  for (let cardIndex = 0; cardIndex < DECK.length; cardIndex++) {
    let currentCard = [...encryptedDeck[cardIndex]];
